...
Entropy.complexity.values <- function(Series.to.analysis, dimension, delay){
  ...
  for(i in c(1:dim(Series.to.analysis)[1])){
    ...
    #prob[i,] <- completeCase(Series.to.analysis[i,], dimension, delay, patterns, elements, symbols) #Blue
    #Entropy[i] <- shannonNormalized(prob[i,])
    #Complexity[i] <- Ccomplexity(prob[i,])
    ...
    prob[i,] <- randomImputation(Series.to.analysis[i,], dimension, delay, patterns, elements, symbols) #Orange
    Entropy[i] <- shannonNormalized(prob[i,])
    Complexity[i] <- Ccomplexity(prob[i,])
...
write.csv(Entropy.Complexity, file = "EntropyComplexity_Random_Imputation1.csv")

Entropy.Complexity <- Entropy.complexity.values(Series.to.analysis[201:400,], dimension, delay)
write.csv(Entropy.Complexity, file = "EntropyComplexity_Random_Imputation2.csv")

Entropy.Complexity <- Entropy.complexity.values(Series.to.analysis[401:600,], dimension, delay)
write.csv(Entropy.Complexity, file = "EntropyComplexity_Random_Imputation3.csv")

Entropy.Complexity <- Entropy.complexity.values(Series.to.analysis[601:800,], dimension, delay)
write.csv(Entropy.Complexity, file = "EntropyComplexity_Random_Imputation4.csv")

Entropy.Complexity <- Entropy.complexity.values(Series.to.analysis[801:1000,], dimension, delay)
write.csv(Entropy.Complexity, file = "EntropyComplexity_Random_Imputation5.csv")
