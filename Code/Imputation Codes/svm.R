...
###############################################################################################################
#Calculando a correlação de Pearson entre os dados
...
train <- subset(Entropy.Complexity.train, Attacked == FALSE)
trainpredictors <- train[, 1:2]

svm.model <- svm(trainpredictors, y = NULL, type = 'one-classification', nu = 0.10, scale = TRUE, kernel = "radial")

svm.predtest <- predict(svm.model, Entropy.Complexity.test)
svm.predtest

###############################################################################################################
#Testando o SVM

train <- subset(Entropy.Complexity.train, Attacked == TRUE)
trainpredictors <- train[, 1:2]
trainLabels <- train[, 3]

svm.model <- svm(subset(Entropy.Complexity.train, Attacked == TRUE)[,1:2], y = Entropy.Complexity.train[,3], type = 'C-classification', nu = 0.10, scale = TRUE, kernel = "radial", cross=10, probability=TRUE)
